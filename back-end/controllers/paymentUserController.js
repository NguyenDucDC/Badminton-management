...
exports.vnpay_ipn = async (req, res, next) => {
            ...
            if (checkAmount) {
                if (paymentStatus == "0") { //kiểm tra tình trạng giao dịch trước khi cập nhật tình trạng thanh toán
                    if (rspCode == "00") {
                        ...
                        res.status(200).json({ RspCode: '00', Message: 'Giao dịch thành công' })
                    }
                    else {
                        //that bai
                        //paymentStatus = '2'
                        // Ở đây cập nhật trạng thái giao dịch thanh toán thất bại vào CSDL của bạn
                        res.status(200).json({ RspCode: '00', Message: 'Giao dịch thất bại' })
                    }
                }
                else {
...
exports.vnpay_refund = async (req, res, next) => {
    process.env.TZ = 'Asia/Ho_Chi_Minh';
    let date = new Date();
    let crypto = require("crypto");

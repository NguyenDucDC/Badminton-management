const orderUserServices = require('../services/orderUserServices');
...
exports.priceCalculation = async (req, res) => {
    try {
        const data = req.body
        const Price = await facilityUserServices.getPriceFacility(data.facility)
...
exports.createOrder = async (req, res) => {
    ...
    try {
        if (data.defaultMonth) {
            ...
            const invalidCourt = await calendarUserServices.checkCalendarDefaultMonth(data, days) // trả về sân bị trùng lịch
            if (invalidCourt.length) {
                ...
                await orderUserServices.createOrderDefaultMonth(data, days); // nếu không bị trùng lịch thì tạo đơn hàng
            }

        } else {
            const invalidCourt = await calendarUserServices.checkCalendar(data) // trả về sân bị trùng lịch
            if (invalidCourt.length) {
                return res.status(500).json({
                    message: 'Đơn hàng bị trùng lịch!',
                    status: 0,
                ...
                await orderUserServices.createOrder(data); // nếu không bị trùng lịch thì tạo đơn hàng
            }
        }
...
exports.getListOrder = async (req, res) => {
    ...
    try {
        const orders = await orderUserServices.getListOrder(id);
...
exports.getDetailOrder = async (req, res) => {
    ...
    try {
        const order = await orderUserServices.getDetailOrder(id);
